...
class SdkRepositoryTests(unittest.TestCase):
    ...
    def test_bootstrap_uses_locked_metadata_and_replaces_obsolete_output(self):
        ...
        sim_sdk = next((self.output / "packages/bonsai_swiftui_ios_simulator_sdk").glob("*/files"))
        sim_manifest = (sim_sdk / "manifest.sexp").read_text()
        self.assertIn("(platform iossimulator)", sim_manifest)
        self.assertIn("(cross_compiler ocaml-ios64-simulator 5.1.1)", sim_manifest)
        sim_compiler = self.output / "packages/ocaml-ios64-simulator/ocaml-ios64-simulator.5.1.1"
        sim_compiler_opam = (sim_compiler / "opam").read_text()
        self.assertIn("iOS Simulator arm64", sim_compiler_opam)
        self.assertIn("conf-ios-simulator", sim_compiler_opam)
        self.assertNotIn('%{conf-ios:', sim_compiler_opam)
        sim_conf = self.output / "packages/conf-ios-simulator/conf-ios-simulator.4/opam"
        sim_conf_text = sim_conf.read_text()
        self.assertIn("conf-ios-simulator.config", sim_conf_text)
        self.assertIn("-simulator", sim_conf_text)
        sim_runtime = next(
            (self.output / "packages/bonsai_swiftui_ios_simulator_runtime_sdk").glob("*/opam")
        ).read_text()
        self.assertIn('"conf-ios-simulator" {= "4"}', sim_runtime)
        self.assertIn('"ocaml-ios64-simulator" {= "5.1.1"}', sim_runtime)
        self.assertNotIn('"conf-ios" {', sim_runtime)
        self.assertNotIn('"ocaml-ios64" {', sim_runtime)
        lock = (self.output / "repository.sexp").read_text()
        self.assertIn("(simulator_sdk_package bonsai_swiftui_ios_simulator_sdk", lock)
        ...
        self.assertIn("conf-ios-simulator|4|local|", universe)
        self.assertIn("conf-pkg-config|5|local|", universe)
        self.assertIn("ocaml-ios64-simulator|5.1.1|local|", universe)
        baseline = self.tree()
