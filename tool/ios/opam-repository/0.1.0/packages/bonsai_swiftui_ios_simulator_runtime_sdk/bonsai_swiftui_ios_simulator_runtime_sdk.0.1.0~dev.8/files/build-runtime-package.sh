...
if test "$package_name" = zarith; then
  ...
  # Native archives must remain linkable after opam removes its build directory.
  # ocamlopt adds this package's -I directory to the native linker's search path.
  cp "$gmp_root/lib/libgmp.a" "$target_package_root/libgmp.a"
...
while IFS= read -r component; do
    ...
    if test -z "$component_archives"; then
      ...
      # A virtual library also owns concrete modules. Its public interface
      # alone does not build the native objects needed by implementations.
      # Use installed host filenames as an inventory, never as target inputs.
      set -- "$virtual_target_prefix/byte/$dune_name.cmi"
      for host_native_metadata in "$component_host_directory"/*.cmx; do
        test -f "$host_native_metadata" || continue
        set -- "$@" "$virtual_target_prefix/native/${host_native_metadata##*/}"
      done
      OPAMROOT="$opam_root" \
        SDK="$sdk_version" \
        VER="$IOS_DEPLOYMENT_TARGET" \
