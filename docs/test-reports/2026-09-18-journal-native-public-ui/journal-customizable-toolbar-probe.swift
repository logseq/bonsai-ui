...
struct DynamicToolbar: CustomizableToolbarContent {
  let groups: [Int]
  func compose(_ groups: ArraySlice<Int>) -> any CustomizableToolbarContent {
    guard let first = groups.first else {
      return ToolbarItem(id: "empty", placement: .primaryAction) { EmptyView() }
    }
...
struct Root: View {
  ...
  var body: some View {
    Text("Stable body").frame(width: 500, height: 300)
      .toolbar(id: "journal-keyed-probe") { DynamicToolbar(groups: state.groups) }
  }
}
