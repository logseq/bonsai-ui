...
@MainActor @Observable private final class ToolbarProbeState {
  ...
  func input(_ node: RenderNodeState, _ payload: NativeEventPayload) {
    if case .focusChanged(true) = payload, let focus, focus.0 !== node {
      self.focus = nil
      if order.contains(where: { $0 === node }) { captureFocus() }
    }
  }
  func mounted() {
...
private struct ToolbarProbeContent: ToolbarContent {
  ...
  var body: some ToolbarContent {
    if let node = nodes.first {
      ToolbarItem(id: "group-\(node.id.node)", placement: placement) {
        ControlGroup {
          ToolbarProbeMount(host: hosts[node.id]!, mounted: mounted)
        }.id(node.id)
      }
      if spacers { ToolbarSpacer(.fixed, placement: placement) }
      ToolbarContentBuilder.buildLimitedAvailability(
        ToolbarProbeContent(
          nodes: Array(nodes.dropFirst()), placement: placement, hosts: hosts,
          mounted: mounted, spacers: spacers))
    }
  }
}
...
@MainActor struct ToolbarIdentityTests {
  ...
  func nativeFrameworkFieldsRetainFocusAcrossKeyedGroupMoves(focused: UInt64) async throws {
    ...
    tree.onInput = { node, payload in state.input(node, payload); return true }
