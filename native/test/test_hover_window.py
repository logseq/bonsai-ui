"""Exercise native Gallery hover overlap ownership, presentation, handlers and underlying actions."""
...
class HoverWindowTests(unittest.TestCase):
    def test_hover_window(self):
        bundle = ROOT / "_build/validation/HoverWindowAcceptance.app"
        executable = bundle / "Contents/MacOS/HoverWindowAcceptance"
        executable.parent.mkdir(parents=True, exist_ok=True)
        (bundle / "Contents/Info.plist").write_bytes(plistlib.dumps({
            "CFBundleIdentifier": "org.bonsai-swiftui.test.hover-window",
        ...
        build = subprocess.run(["xcrun", "swiftc", "-parse-as-library", "-target", "arm64-apple-macos26.0",
            ...
            str(ROOT / "native/test/hover_window.swift"), "-o", str(executable)],
            cwd=ROOT, capture_output=True, text=True)
        self.assertEqual(build.returncode, 0, build.stdout + build.stderr)
        result = subprocess.run([str(executable)], cwd=ROOT, env=os.environ,
                                capture_output=True, text=True, timeout=60)
        self.assertEqual(result.returncode, 0, result.stdout + result.stderr)
        self.assertIn("PASS: actual Gallery Hover ownership", result.stdout)
        print(result.stdout)
