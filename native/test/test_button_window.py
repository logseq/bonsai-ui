"""Exercise actual Gallery buttons actions inside a native SwiftUI App event loop."""
...
class ButtonWindowTests(unittest.TestCase):
    def test_button_window(self):
        bundle = ROOT / "_build/validation/ButtonWindowAcceptance.app"
        executable = bundle / "Contents/MacOS/ButtonWindowAcceptance"
        executable.parent.mkdir(parents=True, exist_ok=True)
        (bundle / "Contents/Info.plist").write_bytes(plistlib.dumps({
            "CFBundleIdentifier": "org.bonsai-swiftui.test.button-window",
        ...
        build = subprocess.run(["xcrun", "swiftc", "-parse-as-library", "-target", "arm64-apple-macos26.0",
            ...
            str(ROOT / "native/test/button_window.swift"), "-o", str(executable)],
            cwd=ROOT, capture_output=True, text=True)
        self.assertEqual(build.returncode, 0, build.stdout + build.stderr)
        result = subprocess.run([str(executable)], cwd=ROOT, env=os.environ,
                                capture_output=True, text=True, timeout=30)
        self.assertEqual(result.returncode, 0, result.stdout + result.stderr)
        self.assertIn("PASS: actual Gallery buttons native actions", result.stdout)
        print(result.stdout)
