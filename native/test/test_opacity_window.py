"""Exercise actual Gallery opacity completions inside a native SwiftUI App event loop."""
...
class OpacityWindowTests(unittest.TestCase):
    def test_opacity_window(self):
        bundle = ROOT / "_build/validation/OpacityWindowAcceptance.app"
        executable = bundle / "Contents/MacOS/OpacityWindowAcceptance"
        executable.parent.mkdir(parents=True, exist_ok=True)
        (bundle / "Contents/Info.plist").write_bytes(plistlib.dumps({
            "CFBundleIdentifier": "org.bonsai-swiftui.test.opacity-window",
        ...
        build = subprocess.run(["xcrun", "swiftc", "-parse-as-library", "-target", "arm64-apple-macos26.0",
            ...
            str(ROOT / "native/test/opacity_window.swift"), "-o", str(executable)],
            cwd=ROOT, capture_output=True, text=True)
        self.assertEqual(build.returncode, 0, build.stdout + build.stderr)
        result = subprocess.run([str(executable)], cwd=ROOT, env=os.environ,
                                capture_output=True, text=True, timeout=60)
        self.assertEqual(result.returncode, 0, result.stdout + result.stderr)
        self.assertIn("PASS: actual Gallery opacity native completions", result.stdout)
        print(result.stdout)
