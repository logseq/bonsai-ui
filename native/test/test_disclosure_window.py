"""Exercise actual Gallery disclosure selection and navigation inside a native SwiftUI App event loop."""
...
class DisclosureWindowTests(unittest.TestCase):
    def test_disclosure_window(self):
        bundle = ROOT / "_build/validation/DisclosureWindowAcceptance.app"
        executable = bundle / "Contents/MacOS/DisclosureWindowAcceptance"
        executable.parent.mkdir(parents=True, exist_ok=True)
        (bundle / "Contents/Info.plist").write_bytes(plistlib.dumps({
            "CFBundleIdentifier": "org.bonsai-swiftui.test.disclosure-window",
        ...
        build = subprocess.run(["xcrun", "swiftc", "-parse-as-library", "-target", "arm64-apple-macos26.0",
            ...
            str(ROOT / "native/test/disclosure_window.swift"), "-o", str(executable)],
            cwd=ROOT, capture_output=True, text=True)
        self.assertEqual(build.returncode, 0, build.stdout + build.stderr)
        result = subprocess.run([str(executable)], cwd=ROOT, env=os.environ,
                                capture_output=True, text=True, timeout=60)
        self.assertEqual(result.returncode, 0, result.stdout + result.stderr)
        self.assertIn("PASS: actual Gallery Disclosure native expand/collapse", result.stdout)
        print(result.stdout)
