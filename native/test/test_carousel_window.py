"""Exercise actual Gallery Carousel actions inside a native SwiftUI App event loop."""
...
class CarouselWindowTests(unittest.TestCase):
    def test_carousel_window(self):
        bundle = ROOT / "_build/validation/CarouselWindowAcceptance.app"
        executable = bundle / "Contents/MacOS/CarouselWindowAcceptance"
        executable.parent.mkdir(parents=True, exist_ok=True)
        (bundle / "Contents/Info.plist").write_bytes(plistlib.dumps({
            "CFBundleIdentifier": "org.bonsai-swiftui.test.carousel-window",
        ...
        build = subprocess.run(["xcrun", "swiftc", "-parse-as-library", "-target", "arm64-apple-macos26.0",
            ...
            str(ROOT / "native/test/carousel_window.swift"), "-o", str(executable)],
            cwd=ROOT, capture_output=True, text=True)
        self.assertEqual(build.returncode, 0, build.stdout + build.stderr)
        result = subprocess.run([str(executable)], cwd=ROOT, env=os.environ,
                                capture_output=True, text=True, timeout=60)
        self.assertEqual(result.returncode, 0, result.stdout + result.stderr)
        self.assertIn("PASS: actual Carousel native viewport changes update OCaml, preserve free scrolling and restore rejected positions", result.stdout)
        print(result.stdout)
