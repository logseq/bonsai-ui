"""Exercise the actual Clock app, foreground timers and presentation waits."""
...
class ClockWindowTests(unittest.TestCase):
    def test_clock_window(self):
        bundle = ROOT / "_build/validation/ClockWindowAcceptance.app"
        executable = bundle / "Contents/MacOS/ClockWindowAcceptance"
        executable.parent.mkdir(parents=True, exist_ok=True)
        (bundle / "Contents/Info.plist").write_bytes(plistlib.dumps({
            "CFBundleIdentifier": "org.bonsai-swiftui.test.clock-window",
            "CFBundleName": "ClockWindowAcceptance", "CFBundleExecutable": executable.name,
        ...
        build = subprocess.run(["xcrun", "swiftc", "-parse-as-library", "-target", "arm64-apple-macos26.0",
            ...
            str(ROOT / "native/test/clock_window.swift"), "-o", str(executable)],
            cwd=ROOT, capture_output=True, text=True)
        self.assertEqual(build.returncode, 0, build.stdout + build.stderr)
        result = subprocess.run([str(executable)], cwd=ROOT, env=os.environ,
                                capture_output=True, text=True, timeout=45)
        self.assertEqual(result.returncode, 0, result.stdout + result.stderr)
        self.assertIn("PASS: actual Clock native window", result.stdout)
        print(result.stdout)
