"""Exercise native Gallery badge counts, visibility, alignment and underlying actions."""
...
class BadgeWindowTests(unittest.TestCase):
    def test_badge_window(self):
        bundle = ROOT / "_build/validation/BadgeWindowAcceptance.app"
        executable = bundle / "Contents/MacOS/BadgeWindowAcceptance"
        executable.parent.mkdir(parents=True, exist_ok=True)
        (bundle / "Contents/Info.plist").write_bytes(plistlib.dumps({
            "CFBundleIdentifier": "org.bonsai-swiftui.test.badge-window",
        ...
        build = subprocess.run(["xcrun", "swiftc", "-parse-as-library", "-target", "arm64-apple-macos26.0",
            ...
            str(ROOT / "native/test/badge_window.swift"), "-o", str(executable)],
            cwd=ROOT, capture_output=True, text=True)
        self.assertEqual(build.returncode, 0, build.stdout + build.stderr)
        result = subprocess.run([str(executable)], cwd=ROOT, env=os.environ,
                                capture_output=True, text=True, timeout=60)
        self.assertEqual(result.returncode, 0, result.stdout + result.stderr)
        self.assertIn("PASS: actual Gallery Badge exact counts", result.stdout)
        print(result.stdout)
