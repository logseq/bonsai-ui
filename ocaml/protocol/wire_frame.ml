...
let validate_native_menu_items items =
  let ids = Hashtbl.create (min 1024 (List.length items)) in
  if List.length items = 0 || List.length items > 1024
  then Error "action menus require between 1 and 1024 items"
  else if
    List.exists
      (fun (item : native_menu_item) ->
         let id = ID.Host.Native_menu_item_id.to_string item.item_id in
         let duplicate = Hashtbl.mem ids id in
         Hashtbl.replace ids id ();
         id = ""
         || duplicate
         || String.trim item.label = ""
         || String.length id > Generated_protocol.Limits.max_string_bytes - 5
         || String.length item.label > Generated_protocol.Limits.max_string_bytes)
      items
  then
    Error
      "action menu IDs must be nonempty and unique, and labels must be nonblank within \
       the string limit"
  else Ok ()
;;
